...
library(survivalROC)
#setwd("   ")      #设置工作目录
rt=read.table("GSE71729Risk.txt",header=T,sep="\t",check.names=F,row.names=1)    #读取cox回归风险文件
bioROC=function(inputFile=null,rocFile=null){
  
  rt=read.table("GSE71729Risk.txt",header=T,sep="\t")
...
bioROC(inputFile="GSE71729Risk.txt",rocFile="geo_ROC.pdf")
